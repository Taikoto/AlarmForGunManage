...
u8 USART2_RX[USART2_RX_LEN+1] = {0};        

u8 USART3_RX[USART3_RX_LEN+1] = {0};       
...
void Usart3IRQ(COMM_USART *p_usart)
{
	//UsartIRQ(&g_usart3);
	u8	*USART_RX = p_usart->rx_buf;
	u8 Res;
    static char i = 0,start = 0;
    if(USART_GetITStatus(p_usart->usart,USART_IT_RXNE) != RESET)  //接收中断
        {
            Res = USART_ReceiveData(p_usart->usart); //读取接收到的数据
            //USART_SendData(USART1,Res);
            if(Res == 0x28) //如果接收的第一位数据是0X28（这个是查看传感器的手册得知的）
            {
                p_usart->rx_num = 0;     //让数组索引值从0开始
                start = 1;  //这个变量是来确定第二位是否接收到了（这个也是查看传感器的手册得知的）
            }

            if(start == 1)
            {
							USART_RX[p_usart->rx_num++] = Res;//(USART1->DR);	//读取接收到的数据
                if(p_usart->rx_num >= 46 && (p_usart->rx_buf[46] == 0x0d))
                    {
                        start  = 0;
                        p_usart->rx_num = 0;//重新开始接收   
                        p_usart->rx_buf[0] = 0;
                    }
                }                   
        }
